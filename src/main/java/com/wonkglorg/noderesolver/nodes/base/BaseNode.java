...
public abstract class BaseNode implements Cloneable {
    ...
    protected String formatClassError(Class<?> expectedClass, Class<?> providedClass, boolean simpleName) {
        ...
        if (expectedClass.isAssignableFrom(providedClass)) {
            return "Input expected type: \"%s\" but found: \"%s\".".formatted(expectedTypeName, providedTypeName);
        }
        ...
        if (expectedClass.isPrimitive() || providedClass.isPrimitive()) {
            if (getWrapperType(expectedClass).equals(getWrapperType(providedClass))) {
                return "Input expected type: \"%s\" but found: \"%s\" (both primitive types).".formatted(expectedTypeName, providedTypeName);
            }
        }
    ...
    protected <T> T resolveInput(Object input, Class<T> type) {
        if (!type.isInstance(input)) {
            throw new IllegalStateException(formatClassError(type, input == null ? null : input.getClass()));
        }
